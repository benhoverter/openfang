...
//!
//! - **No vocabulary enforcement.** `claim_key` is a controlled vocabulary
//!   (§2.3.3 mitigation 1), but the gate belongs at the writer, where a
//!   rejection can be explained to the model that chose the key. A storage
//!   layer that silently refused a key would make a slot look empty when it is
//!   not. That is ANAI-204 step 4.
    ...
    pub fn upsert(&self, write: FactWrite) -> OpenFangResult<FactOutcome> {
        ...
        let live = tx
            .query_row(
                ...
                rusqlite::params![agent, write.scope, write.claim_key, KIND_FACT],
                |r| {
                    Ok(LiveRow {
                        id: r.get(0)?,
                        claim: r.get(1)?,
