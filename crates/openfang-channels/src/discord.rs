            ...
            ChannelContent::Multipart(parts) => {
                ...
                for part in parts {
                    match part {
                        ...
                        ChannelContent::Interactive { .. } => unknown_names.push("Interactive"),
                    }
                }
...
fn chunk_attachments(
    ...
    for item in attachments {
        ...
        let would_exceed_bytes = current_bytes.saturating_add(item_len) > CHUNK_TOTAL_CAP_BYTES;
...
mod tests {
    ...
    fn test_adapter() -> DiscordAdapter {
        DiscordAdapter::new("test-token".into(), vec![], vec![], true, 0, String::new())
    }
    ...
    async fn spawn_429_then_ok_stub(
        first_response: Arc<dyn Fn() -> axum::response::Response + Send + Sync + 'static>,
                    ...
                    move |Extension(_): Extension<()>, mut multipart: Multipart| {
                        ...
                        async move {
                            ...
                            while let Ok(Some(field)) = multipart.next_field().await {
                                let name = field.name().unwrap_or("").to_string();
                                if name == "payload_json" {
                                    post_rec.payload_json = field.text().await.unwrap_or_default();
                                } else {
                                    let filename = field.file_name().map(str::to_string);
                                    let content_type = field.content_type().map(str::to_string);
                                    let _ = field.bytes().await;
